...
int Gh3x2xDemoInit(void)
{
    ...
    GH3X2X_RegisterHookFunc(gh3x2x_init_hook_func, 
                            ...
                            gh3x2x_get_rawdata_hook_func,				//采样波形
                            
														//gh3x2x_frame_data_hook_func,
														gh3x2x_algorithm_get_io_data_hook_func,
...
//    GH3X2X_RegisterGetBootloaderVersionFunc(GetEvkBootloaderVersion);																					
    ...
    /* Step 5: init algo SDK模块初始化*/

		#if (__DRIVER_LIB_MODE__ == __DRV_LIB_WITH_ALGO__)

		GH3X2X_AlgoMemoryConfig();	//初始化内存池
    GH3X2X_RegisterFuncInfoStruct(g_pstGh3x2xFrameInfo);	//结构体注册
...
//                GH3X2X_SoftLedADJAutoADJInt();
        ...
        if (usGotEvent & GH3X2X_IRQ_MSK_LEAD_ON_DET_BIT)
        {
			...
			send_data[0] = SERVICE_EVT_LEAN_ON;
        ...
        if (usGotEvent & GH3X2X_IRQ_MSK_LEAD_OFF_DET_BIT)
        {
			...
			send_data[0] = SERVICE_EVT_LEAN_OFF;
			osal_queue_send( app_service_evt_queue, send_data, OSAL_MAX_DELAY);
