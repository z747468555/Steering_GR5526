...
#define DISPLAY_RESET_IO_PIN  APP_IO_PIN_12
#define DISPLAY_RESET_IO_MUX  APP_IO_MUX



#define DISPLAY_BL_IO_TYPE APP_IO_TYPE_GPIOA
#define DISPLAY_BL_IO_PIN  APP_IO_PIN_8
#define DISPLAY_BL_IO_MUX  APP_IO_MUX

#define DISPLAY_POWER_EN_IO_TYPE  APP_IO_TYPE_GPIOA
#define DISPLAY_POWER_EN_IO_PIN   APP_IO_PIN_2
#define DISPLAY_BL_IO_MUX  APP_IO_MUX
 ...
 * Global Functions
*/
void lcd_lowpower(void)
{
	dc_spi_send_sync(0x10, NULL, 0);
}
void graphics_dc_st77916_init(uint16_t screen_w, uint16_t screen_h)
{
    ...
    app_graphics_dc_params_t dc_params = {
        ...
        .clock_freq = GDC_CLOCK_FREQ_24MHz,
	...
	//lcd power pin
	io_init.pin = DISPLAY_POWER_EN_IO_PIN;
    io_init.mode = APP_IO_MODE_OUTPUT;
    io_init.pull = APP_IO_NOPULL;
    io_init.mux = DISPLAY_RESET_IO_MUX;
    app_io_init(DISPLAY_POWER_EN_IO_TYPE, &io_init);
    app_io_write_pin(DISPLAY_POWER_EN_IO_TYPE, DISPLAY_POWER_EN_IO_PIN, APP_IO_PIN_SET);
...
void graphics_dc_st77916_set_on(bool on)
{
    ...
    if (on)
    {
        APP_LOG_INFO("Send 29h\n");
        dc_spi_send_sync(0x29, NULL, 0);
        APP_LOG_INFO("Backlight on\n");
        app_io_write_pin(DISPLAY_BL_IO_TYPE, DISPLAY_BL_IO_PIN, APP_IO_PIN_SET);
    }
    else
    {
        APP_LOG_INFO("Send 28h\n");
        dc_spi_send_sync(0x28, NULL, 0);
        APP_LOG_INFO("Backlight off\n");
...
void graphics_dc_st77916_wait_te(void)
{
    if (s_te_valid)
    {
        ...
        if (OSAL_SUCCESS != ret)
        {
            ...
            s_te_valid = false;
			APP_LOG_INFO("TE invalid");
        }
		else
		{
			//APP_LOG_INFO("TE valid");
		}
#else // USE_OSAL
...
void st77916_lowpower(void)
{
    dc_spi_send_sync(0x10, NULL, 0);  // Sleep In命令
    delay_ms(50);  // 等待LCD进入睡眠状态
//    app_io_write_pin(DISPLAY_POWER_EN_IO_TYPE, DISPLAY_POWER_EN_IO_PIN, APP_IO_PIN_SET);
}
void st77916_poweron(void)
{
//    app_io_write_pin(DISPLAY_POWER_EN_IO_TYPE, DISPLAY_POWER_EN_IO_PIN, APP_IO_PIN_RESET);
//    delay_ms(50);  // 等待电源稳定
    dc_spi_send_sync(0x11, NULL, 0);  // Sleep Out命令
    delay_ms(120);  // 等待LCD从睡眠中唤醒
}
...
static void st77916_init_sequence(void)
{
    ...
    datalen = 0;
    data[datalen++] = 0xf0;
    data[datalen++] = 0x06;//0x0B;
    data[datalen++] = 0x0b;//0x12;
    data[datalen++] = 0x0a;//0x0B;
    data[datalen++] = 0x0A;
    data[datalen++] = 0x06;
    data[datalen++] = 0x34;//0x39;
    data[datalen++] = 0x44;//0x43;
    data[datalen++] = 0x4b;//0x4F;
    data[datalen++] = 0x09;//0x07;
    data[datalen++] = 0x16;//0x14;
    data[datalen++] = 0x2f;//0x14;
    data[datalen++] = 0x35;//0x2f;
    //data[datalen++] = 0x34;
    ...
    data[datalen++] = 0xf0;
    data[datalen++] = 0x06;//0x0B;
    data[datalen++] = 0x0b;//0x11;
    data[datalen++] = 0x09;//0x0A;
    data[datalen++] = 0x08;//0x09;
    data[datalen++] = 0x06;//0x05;
    data[datalen++] = 0x2E;//0x32;
    data[datalen++] = 0x33;
    data[datalen++] = 0x46;//0x48;
    data[datalen++] = 0x08;//0x07;
    data[datalen++] = 0x14;//0x13;
    data[datalen++] = 0x13;
    data[datalen++] = 0x2d;//0x2C;
    data[datalen++] = 0x32;//0x33;
    ...
    datalen = 0;
    data[datalen++] = 0x00;//0x12;
    dc_spi_send_sync(0xC0, data, datalen);

    datalen = 0;
    data[datalen++] = 0x54;//0x88;
    dc_spi_send_sync(0xC1, data, datalen);

    datalen = 0;
    data[datalen++] = 0x45;//0x65;
    dc_spi_send_sync(0xC2, data, datalen);

    datalen = 0;
    data[datalen++] = 0x76;//0x74;
    dc_spi_send_sync(0xC3, data, datalen);

    datalen = 0;
    data[datalen++] = 0x67;//0x47;
    dc_spi_send_sync(0xC4, data, datalen);

    datalen = 0;
    data[datalen++] = 0xAA;//0x56;
    dc_spi_send_sync(0xC5, data, datalen);

    datalen = 0;
    data[datalen++] = 0xBB;//0x00;
    dc_spi_send_sync(0xC6, data, datalen);

    datalen = 0;
    data[datalen++] = 0x33;//0xAA;
    dc_spi_send_sync(0xC7, data, datalen);

    datalen = 0;
    data[datalen++] = 0x22;//0xBB;
    dc_spi_send_sync(0xC8, data, datalen);

    datalen = 0;
    data[datalen++] = 0x11;//0x33;
    dc_spi_send_sync(0xC9, data, datalen);

    datalen = 0;
    data[datalen++] = 0x00;//0x21;
    dc_spi_send_sync(0xD0, data, datalen);
